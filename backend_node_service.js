...
// Use the port assigned by the hosting provider, or default to 8000 locally
const PORT = process.env.PORT || 8000;

app.use(cors());
app.use(express.json());

// Ayurvedic Mock Inventory mimicking the data on your storefront
...
// Route to serve the frontend directly from the server root
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'index.html'));
});

// Endpoint serving the botanical product list
app.get('/api/inventory', (req, res) => {
    res.json(herbalInventory);
});

// Post endpoint logging user order logs
app.post('/api/checkout-session', (req, res) => {
    const { items } = req.body;
    
    console.log("\n🌿 =================================================== 🌿");
    console.log("             [AUSHADHI KUNJ HARBAL - CLOUD GATEWAY]            ");
    console.log("🌿 =================================================== 🌿");
    console.log(`⏱️ UTC: ${new Date().toUTCString()}`);
...
// Startup listener supporting cloud-based processes
app.listen(PORT, () => {
    console.log(`\n💚 Store Application Online!`);
    console.log(`🌍 Server dynamically listening on port: ${PORT}`);
    console.log(`👉 Access points resolved locally or in the cloud context.\n`);
});